...
export function shownPoints(object: ObjectFrame): Vec3[] {
  ...
  if (object.strokeRange) {
    ...
    points = [along(points, start), ...points.filter((_, i) => distances[i] > length * start && distances[i] < length * end), along(points, end)];
  }
...
export function strokePaths(object: ObjectFrame): Vec3[][] {
  if (object.geometry.breaks) {
    const ends = [...object.geometry.breaks.slice(1), object.geometry.points.length];
    return object.geometry.breaks.map((start, i) => shownPoints({ ...object,
      geometry: { ...object.geometry, breaks: undefined, points: object.geometry.points.slice(start, ends[i]) },
    }));
  }
  const points = shownPoints(object), dash = object.geometry.dash;
  if (!dash || points.length < 2) return [points];
  const total = points.slice(1).reduce((sum, p, i) => sum + Math.hypot(...p.map((x, k) => x - points[i][k])), 0);
  if (!Number.isFinite(total) || total <= 0) return [];
  const period = Math.max(dash.length / dash.ratio, total / 2048);
  const paths: Vec3[][] = [];
  let travelled = 0;
  for (let i = 1; i < points.length; i++) {
    const a = points[i - 1], b = points[i], length = Math.hypot(...b.map((x, k) => x - a[k]));
    if (!length) continue;
    const end = travelled + length;
    for (let j = Math.floor(travelled / period); j * period < end; j++) {
      const start = Math.max(travelled, j * period), stop = Math.min(end, (j + dash.ratio) * period);
      if (stop > start) paths.push([start, stop].map(distance =>
        a.map((x, k) => x + (b[k] - x) * (distance - travelled) / length) as Vec3));
    }
    travelled = end;
  }
  return paths;
...
export function arrowTips(object: ObjectFrame, points = shownPoints(object)): [Vec3, Vec3][] {
  if (object.strokeRange) return [];
  if (object.geometry.arrows && object.reveal > 0)
    return strokePaths(object).filter(path => path.length > 1)
      .map(path => [path.at(-1)!, path.at(-2)!]);
  if (points.length < 2 || object.reveal <= 0) return [];
  const both = object.type === "DoubleArrow" || object.type === "CurvedDoubleArrow";
  if (!both && object.type !== "Arrow" && object.type !== "CurvedArrow") return [];
  return [[points.at(-1)!, points.at(-2)!], ...(both ? [[points[0], points[1]] as [Vec3, Vec3]] : [])];
}
