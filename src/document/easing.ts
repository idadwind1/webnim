...
export const easingNames = ["linear", "smooth", "ease-in", "ease-out", "smoother", "sine-in", "sine-out", "sine-in-out", "cubic-in", "cubic-out", "cubic-in-out", "quart-in", "quart-out", "quart-in-out", "quint-in", "quint-out", "quint-in-out", "expo-in", "expo-out", "expo-in-out", "circ-in", "circ-out", "circ-in-out"] as const;
...
export function easeProgress(p: number, easing: string = "linear"): number {
  ...
  const base = (x:number):number => {
    ...
    const power=easing.startsWith("cubic")?3:easing.startsWith("quart")?4:easing.startsWith("quint")?5:1;
    return x**power;
  };
  if(easing.endsWith("in-out")) return p<.5?base(2*p)/2:1-base(2-2*p)/2;
